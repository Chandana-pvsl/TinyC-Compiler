...
postfix_expression:
			...
			|postfix_expression '[' expression ']' 
			{
				...
				$$->loc = gentemp(new type("INTEGER"));//store computed address

				// New address = already computed + $3 * new width
				if($1->cat=="ARR")// if something already computed
				{
					sym_det* t = gentemp(new type("INTEGER"));
					stringstream strs;
		    		strs <<size_type($$->tp);
		    		string temp_str = strs.str();
		    		char* intstr = (char*) temp_str.c_str();
			...
			{
				$$ = new array();
				$$->array = gentemp(new type("INTEGER"));
				$$->loc = gentemp(new type("INTEGER"));
			} 	
			|'(' type_name ')' '{'initializer_list ',' '}' 	
			{
				$$ = new array();
...
unary_expression:
			...
			|unary_operator cast_expression 
			{
				...
				{
					$$->array = gentemp(new type($2->array->tp->sym_type));
